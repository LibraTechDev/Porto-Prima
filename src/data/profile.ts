  ...
  projects: [
    {
      ...
      featured: true,
      category: {
        id: "Sistem Terdistribusi & IoT Fleet",
        en: "Distributed Systems & Fleet IoT",
      },
      tagline: {
      ...
      description: {
        id: "Aplikasi web manajemen armada multi-role dengan pemisahan beban transaksional (OLTP) dan analitik (OLAP). FastAPI dan PostgreSQL menangani CRUD data operasional (kendaraan, sopir, trip, dan autentikasi JWT). Microservice Rust dengan mesin DuckDB in-process mengeksekusi agregasi log telemetri armada. Antarmuka web dibuat dengan Vue.js 3 dan Pinia, di-deploy secara otomatis (Automated CI/CD) ke server Linux VPS via Docker, reverse proxy Nginx, dan pemantauan trafik GoAccess.",
        en: "Multi-role fleet management web app separating transactional (OLTP) and analytical (OLAP) workloads. FastAPI and PostgreSQL handle operational CRUD (fleet assets, drivers, trips, and JWT auth). A Rust service with in-process DuckDB runs aggregate telemetry queries. The web UI is built with Vue.js 3 and Pinia, deployed via Automated CI/CD pipeline to a Linux VPS via Docker behind an Nginx reverse proxy with GoAccess traffic monitoring.",
      },
      tech: [
        "Vue.js 3",
        "Pinia Store",
      ...
      architecture: {
        frontend: "Vue.js 3 SPA with Realtime Telemetry Dashboard",
        ...
        service: "Rust Microservice (High-throughput metric ingestion & crunching)",
        olap: "DuckDB In-Memory OLAP (Fast analytical queries over massive logs)",
        ops: [
